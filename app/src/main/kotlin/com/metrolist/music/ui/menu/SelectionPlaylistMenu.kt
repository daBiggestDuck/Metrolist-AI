...
fun SelectionPlaylistMenu(
    ...
    val allLiked =
        remember(allSongs) {
            allSongs.isNotEmpty() && allSongs.all { it.song.liked }
        }
                        ...
                        if (allSongs.isNotEmpty()) {
                            add(
                                Material3MenuItemData(
                                    ...
                                    description = {
                                        Text(text = stringResource(R.string.add_selection_to_liked_desc))
                                    },
                                    icon = {
                                        Icon(
                                            painter =
                                    ...
                                    onClick = {
                                        val currentlyAllLiked = allSongs.all { it.song.liked }
                                        onDismiss()
                                        database.query {
                                            allSongs.forEach { song ->
                                                if ((!currentlyAllLiked && !song.song.liked) || currentlyAllLiked) {
                                                    val updated = song.song.toggleLike()
                                                    update(updated)
                            ...
                            add(
                                Material3MenuItemData(
                                    title = { Text(text = stringResource(R.string.add_to_bad_taste)) },
                                    description = {
                                        Text(text = stringResource(R.string.add_to_bad_taste_desc))
                                    },
                                    icon = {
                                        Icon(
                                    ...
                                    onClick = {
                                        val songIds = allSongs.map { it.id }.distinct()
                                        onDismiss()
                                        coroutineScope.launch(Dispatchers.IO) {
